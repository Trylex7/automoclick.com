...
if (empty($numero_client) && !empty($client_email)) {
    $stmt = $db->prepare('
        ...
        FROM login_user 
        WHERE email = ? 
          AND numero_client IN (SELECT numero_client FROM rdvs WHERE numero_pro = ?)
    ');
    $stmt->execute([$client_email, $numero_pro]); // <- ajouter $numero_pro ici
    ...
    $devis_numero = strtoupper(uniqid('FC-'));
    $html = '
<html>
<head>
    <meta charset="UTF-8">
    <style>
        ...
        .footer { margin-top: 160px; font-size: 13px; }
    </style>
</head>
<body>
    <h1>FACTURE</h1>
    ...
    <div class="info-box">
        ...
        <p><strong>N° de facture:</strong> ' . $devis_numero . '</p>
...
<div class="footer">
    <p class="right"><strong>Facture générer par <a href="https://www.automoclick.com"style="color: #16a34a;">Automoclick</a> - www.automoclick.com</strong></p>
</div>
</body>
</html>';
...
<body class="bg-gray-100 min-h-screen text-gray-800 px-4 py-8">
    ...
    <div class="max-w-5xl mx-auto bg-white shadow-md rounded-xl p-8 space-y-6">
        <h1 class="text-3xl font-bold text-center text-green-600">Créer votre facture</h1>
